...
export function renderCartOrderPlacedEmail(args: CartOrderPlacedArgs): {
  ...
  const body =
    ...
    emailParagraph(
      `Your payment is authorized, not yet taken &mdash; we&rsquo;ll only take it once your prints enter production. We&rsquo;ll email your invoice with that charge, and send tracking details as soon as your order ships.`,
    ) +
    emailDivider() +
